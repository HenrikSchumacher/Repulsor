...
namespace Repulsor
{
    ...
    public:
        ...
        virtual ExtReal value( const Mesh_T & M ) const override
        {
            TP_AllPairs_Traversor<
                DOM_DIM,DOM_DIM,ClusterTree_T,true,
                true,false,false,false
            >
            traversor(
                M.GetClusterTree(), M.GetClusterTree(), this->metric_values, q, p
            );
    ...
    public:
        ...
        {
            return TOOLS_TO_STD_STRING(CLASS)+"<"+ToString(DOM_DIM)+","+ToString(AMB_DIM)+","+TypeName<Real>+","+TypeName<Int>+","+TypeName<LInt>+","+TypeName<SReal>+","+TypeName<ExtReal>+">("+ToStringFPGeneral(q)+","+ToStringFPGeneral(p)+")";
        }
